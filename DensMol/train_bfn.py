...
def get_dataloader(cfg):
    if cfg.data.name == 'pl_tr':
        ...
        # 如果开启 ED feature，把 ed_dim 加到 protein feature dim 上
        # if hasattr(cfg, "ed") and getattr(cfg.ed, "enabled", False):
        #     cfg.dynamics.protein_atom_feature_dim += int(cfg.ed.ed_dim)
    else:
...
if __name__ == "__main__":
    ...
    # #############################################################
    # print("=" * 80)
    # print("[CHECK] cfg.debug =", cfg.debug)
    # print("[CHECK] len(train_loader.dataset) =", len(train_loader.dataset))
    # print("[CHECK] len(val_loader.dataset) =", len(val_loader.dataset))
    # print("[CHECK] len(test_loader.dataset) =", len(test_loader.dataset))
    #
    # import os
    #
    #
    # def get_pocket_id_from_filename(fn):
    #     return os.path.basename(fn)[:4]
    #
    #
    # # 统计前 2000 个训练样本里有多少个不同 pocket
    # sample_n = min(len(train_loader.dataset), 2000)
    # pocket_ids = []
    # ligand_files = []
    #
    # for i in range(sample_n):
    #     item = train_loader.dataset[i]
    #     ligand_files.append(item.ligand_filename)
    #     pocket_ids.append(get_pocket_id_from_filename(item.ligand_filename))
    #
    # unique_pockets = sorted(set(pocket_ids))
    # print("[CHECK] sampled train items =", len(pocket_ids))
    # print("[CHECK] unique train pockets in first sample_n =", len(unique_pockets))
    # print("[CHECK] first 20 unique pocket ids =", unique_pockets[:20])
    # print("[CHECK] first 20 ligand filenames =", ligand_files[:20])
    #
    # # 看第一批是不是全是同一个 pocket
    # first_train_batch = next(iter(train_loader))
    # print("[CHECK] first batch ligand_filename =", first_train_batch.ligand_filename)
    # print("[CHECK] first batch pocket ids =",
    #       [get_pocket_id_from_filename(x) for x in first_train_batch.ligand_filename])
    #
    # print("=" * 80)
    #
    #
    ...
    # print("=" * 80)
    # print("[SMOKE TEST] has local_ed =", hasattr(first_batch, "local_ed"))
    # if hasattr(first_batch, "local_ed"):
    #     print("[SMOKE TEST] local_ed.shape =", tuple(first_batch.local_ed.shape))
    #     print("[SMOKE TEST] local_ed.has_nan =", torch.isnan(first_batch.local_ed).any().item())
    #     print("[SMOKE TEST] local_ed.has_inf =", torch.isinf(first_batch.local_ed).any().item())
    # print("=" * 80)
