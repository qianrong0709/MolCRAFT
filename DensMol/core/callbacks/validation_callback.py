...
# # 为了训练，重构的轻量级validation，上一个注释掉的是原来的
# class ValidationCallback(Callback):
#     def __init__(self, dataset, atom_enc_mode, atom_decoder, atom_type_one_hot, single_bond, docking_config, val_freq) -> None:
#         super().__init__()
#         self.dataset = dataset
...
#         self.single_bond = single_bond
#         self.type_one_hot = atom_type_one_hot
#         self.docking_config = copy.deepcopy(docking_config) if docking_config is not None else None
...
#     def setup(self, trainer: Trainer, pl_module: LightningModule, stage: str) -> None:
#         super().setup(trainer, pl_module, stage)
#         # 训练阶段只做 recon loss，不初始化重型 metric
#         self.metric = None
#
#     def on_train_batch_start(
#         self,
...
#
#     @torch.no_grad()
#     def calc_recon_loss(self, trainer: Trainer, pl_module: LightningModule):
#         with torch.no_grad():
#             pl_module.dynamics.eval()
#             sum_batches, sum_loss, sum_loss_pos, sum_loss_type = 0, 0., 0., 0.
#             pos_normalizer = torch.tensor(
#                 pl_module.cfg.data.normalizer_dict.pos,
#                 dtype=torch.float32,
...
#                     ).repeat(num_graphs, 1).index_select(0, batch_ligand)
#
#                     if not pl_module.cfg.dynamics.use_discrete_t and not pl_module.cfg.dynamics.destination_prediction:
#                         t = torch.clamp(t, min=pl_module.dynamics.t_min)
#
#                     c_loss, d_loss, discretised_loss = pl_module.dynamics.reconstruction_loss_one_step(
#                         t,
#                         protein_pos=protein_pos,
...
#
#         if trainer.global_step > 0 and trainer.global_step % self.val_freq == 0:
#             recon_loss = self.calc_recon_loss(trainer, pl_module)
#             pl_module.log_dict(
#                 recon_loss,
#                 on_step=True,
