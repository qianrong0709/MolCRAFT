...
from core.datasets.ed_query import query_local_ed  # ED查询函数
...
class PocketLigandPairDataset(Dataset):
    ...
    def __init__(
        ...
        # ===== ED query config =====
        self.use_ed = use_ed
        self.ed_resolution = ed_resolution
        self.ed_k = ed_k
        self.ed_features = ed_features
        self.ed_use_delta_xyz = ed_use_delta_xyz

        # 自动从 raw_path 推断 pocket 后缀，比如 pocket8 / pocket10
        base_name = os.path.basename(self.raw_path)
        if 'pocket8' in base_name:
            self.ed_pocket_suffix = 'pocket8'
        elif 'pocket10' in base_name:
            self.ed_pocket_suffix = 'pocket10'
        else:
            self.ed_pocket_suffix = None
    ...
    # 新增函数
    def _sample_to_h5_path(self, data):
        """
        根据样本里的 ligand_filename，推导对应的 h5 路径
        """
        if self.ed_pocket_suffix is None:
            raise ValueError(f'Cannot infer pocket suffix from raw_path: {self.raw_path}')

        ligand_filename = data.ligand_filename
        ligand_path = os.path.join(self.raw_path, ligand_filename)
        base = ligand_path[:-4]  # 去掉 .sdf
        h5_path = f"{base}_{self.ed_pocket_suffix}_multires_pointcloud.h5"
        return h5_path
    ...
    def _process(self):
        ...
        num_skipped = 0
        with db.begin(write=True, buffers=True) as txn:  # txn 是 LMDB 的写入事务（transaction）
    ...
    def __getitem__(self, idx):
        ...
        # ===== 在这里把 ED query 接到数据流里 =====
        if self.use_ed:
            h5_path = self._sample_to_h5_path(data)

            local_ed = query_local_ed(
                protein_pos=data.protein_pos,
                h5_path=h5_path,
                resolution=self.ed_resolution,
                k=self.ed_k,
                features=self.ed_features,
                use_delta_xyz=self.ed_use_delta_xyz,
            )

            # 挂到 data 上，后面模型/encoder 直接用
            data.local_ed = local_ed.float()

            # print("=" * 80)
            # print("[DATASET] idx =", idx)
            # print("[DATASET] protein_pos.shape =", tuple(data.protein_pos.shape))
            # print("[DATASET] h5_path =", h5_path)
            # print("[DATASET] local_ed.shape =", tuple(local_ed.shape))
            # print("[DATASET] local_ed.has_nan =", torch.isnan(local_ed).any().item())
            # print("[DATASET] local_ed.has_inf =", torch.isinf(local_ed).any().item())
            # print("=" * 80)
...
class PocketLigandPairDatasetFromComplex(Dataset):
    def __init__(self, raw_path, transform=None, version='final', radius=8.0):   #default: radius = 10
        super().__init__()
        self.raw_path = raw_path.rstrip('/')
        self.index_path = os.path.join(self.raw_path, 'index.pkl')
        base_name = os.path.basename(self.raw_path)
...
class PocketLigandPairDatasetFeaturized(Dataset):
    def __init__(self, raw_path, ligand_atom_mode, version='simple'):
        ...
        if not os.path.exists(self.transformed_path):
            ...
            self._transform()  # 如果 .pt 文件不存在 → 调用 _transform() 做数据处理
        else:   # 如果 .pt 存在 → 直接加载处理好的训练/测试集列表（是 ProteinLigandData 对象）
    ...
    def _transform(self):
        ...
        transform = Compose(transform_list) # 是一个可调用对象（函数或类实例），输入是一个结构数据对象 ProteinLigandData，输出也是一个结构数据对象，但增加了特征字段
...
if __name__ == '__main__':
    ...
    # dataset = PocketLigandPairDataset('./data/crossdocked_v1.1_rmsd1.0_pocket8')
    ...
    print("protein_pos.shape =", tuple(data.protein_pos.shape))
    print("local_ed.shape =", tuple(data.local_ed.shape))
    print("has_nan =", torch.isnan(data.local_ed).any().item())
    print("has_inf =", torch.isinf(data.local_ed).any().item())
