...
def center_pos(protein_pos, ligand_pos, batch_protein, batch_ligand, mode="protein"):
    if mode == "none":
        ...
        offset = scatter_mean(protein_pos, batch_protein, dim=0) #  计算每个蛋白质分子的质心位置
...
class SBDDTrainLoop(pl.LightningModule):
    def __init__(self, config: Config):
        super().__init__()
        self.cfg = config
        self.dynamics = BFN4SBDDScoreModel(**self.cfg.dynamics.todict())    # receiver的主干网络
        # [ time, h_t, pos_t, edge_index]
        self.train_losses = []
        self.save_hyperparameters(self.cfg.todict())    # 保存超参
    ...
    def training_step(self, batch, batch_idx):  # 训练一个batch的逻辑
        t1 = time()
        # protein_pos, protein_v, batch_protein, ligand_pos, ligand_v, batch_ligand = (
        #     batch.protein_pos,      # 蛋白原子坐标
        #     batch.protein_atom_feature.float(), # 蛋白原子特征向量
        #     batch.protein_element_batch,  # 蛋白原子所属分子编号
        #     batch.ligand_pos,      # 配体原子坐标
        #     batch.ligand_atom_feature_full, # 配体原子特征向量
        #     batch.ligand_element_batch, # 配体原子所属分子编号
        ...
        local_ed = getattr(batch, 'local_ed', None)
        if local_ed is not None:
            local_ed = local_ed.float()

            # print("=" * 80)
            # print("[BATCH] has local_ed =", local_ed is not None)
            # if local_ed is not None:
            #     print("[BATCH] local_ed.shape =", tuple(local_ed.shape))
            #     print("[BATCH] local_ed.has_nan =", torch.isnan(local_ed).any().item())
            #     print("[BATCH] local_ed.has_inf =", torch.isinf(local_ed).any().item())
            # print("=" * 80)
        ...
        # protein_pos: [N_pro,3]                    蛋白原子坐标
        # protein_v: [N_pro,27]                     蛋白原子特征向量
        # batch_protein: [N_pro]                    
        # ligand_pos: [N_lig,3]                     配体原子坐标
        # ligand_v: [N_lig,13]                      配体原子特征向量
        ...
        with torch.no_grad():
            ...
            if self.cfg.train.random_rot:
                ...
                Q, __ = np.linalg.qr(M) # 生成一个随机正交矩阵
                Q = torch.from_numpy(Q.astype(np.float32)).to(ligand_pos.device)
                gt_protein_pos = gt_protein_pos @ Q # 对蛋白质坐标做旋转
                ligand_pos = ligand_pos @ Q     # 对配体坐标做旋转
        ...
        """
        用于记录时间戳的变量，配合 time() 来衡量训练中各个阶段的执行耗时，
        是手动 profiling 的一种手段，可以帮助分析性能瓶颈和训练开销
        """
