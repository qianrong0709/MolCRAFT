...
class SinusoidalPosEmb(nn.Module):
    """
    标准Transformer里使用的时间嵌入方法
    构造连续时间步的非线性编码
    将标量时间 t（如 t = 0.2）映射成一个向量，让神经网络能“感知”时间
    """
...
class TimeEmbedLayer(nn.Module):
    ...
    def __init__(self, time_emb_mode, time_emb_dim):
        ...
        if self.time_emb_mode == "simple":  # 恒等映射，模型只看到“当前时间步的数值”
            assert self.time_emb_dim == 1
            self.time_emb = lambda x: x
        elif self.time_emb_mode == "sin":   # sin/cos 嵌入 + MLP
        ...
        elif self.time_emb_mode == "rbf":   # RBF嵌入
            self.time_emb = RBF(0, 1, self.time_emb_dim)
        elif self.time_emb_mode == "rbfnn": # RBF + MLP
            self.time_emb = nn.Sequential(
...
class BFN4SBDDScoreModel(BFNBase):
    def __init__(
        ...
        time_emb_dim=1,
        center_pos_mode='protein',  # self.center_pos_mode, pos_init_mode, destination_prediction 控制采样行为
        pos_init_mode='zero',
        ...
        #
        # self.node_indicator = node_indicator    # 添加一个 node type 标识位（0=protein, 1=ligand）
        #
        ...
        #
        # # atom embedding
        # self.protein_atom_emb = nn.Linear(protein_atom_feature_dim, emb_dim) # 投影蛋白原子特征到 hidden_dim
        ...
        # ===== ED config =====
        self.use_ed = use_ed
        self.ed_input_dim = ed_input_dim
        self.ed_hidden_dim = ed_hidden_dim
        self.ed_dim = ed_dim
        self.ed_pool = ed_pool
        self.ed_k = ed_k

        # ===== ED encoder =====
        if self.use_ed:
            self.ed_encoder = EDPointNetEncoder(
                input_dim=self.ed_input_dim,
                hidden_dim=self.ed_hidden_dim,
                ed_dim=self.ed_dim,
                pool=self.ed_pool,
            )

        # ===== atom embedding =====
        protein_input_dim = protein_atom_feature_dim + (self.ed_dim if self.use_ed else 0)
        self.protein_atom_emb = nn.Linear(protein_input_dim, emb_dim)  # 投影蛋白原子特征到 hidden_dim
        self.center_pos_mode = center_pos_mode  # ['none', 'protein']
        # 替换结束


        # 时间 embedding 会作为 ligand 输入特征一部分
        self.time_emb_mode = time_emb_mode
        self.time_emb_dim = time_emb_dim
        ...
        # 用配体原子节点的特征来预测原子节点的类别logits
        self.v_inference = nn.Sequential(
            nn.Linear(self.hidden_dim, self.hidden_dim),
            ShiftedSoftplus(),  # 定制的激活函数
            nn.Linear(self.hidden_dim, ligand_atom_feature_dim), # 0:3]: 三维坐标均值 μ；[3:6]: 三维坐标 log-std（ρ 或者 logσ）；[6:13]: 原子类型分类 logits（例如 C/N/O/F/Cl...）
        )  # [hidden to 13]
        ...
        self.sampling_strategy = sampling_strategy  # 控制是否使用 vanilla 或端点反馈的策略

        # print("=" * 80)
        # print("[INIT] use_ed =", self.use_ed)
        # if self.use_ed:
        #     print("[INIT] ed_input_dim =", self.ed_input_dim)
        #     print("[INIT] ed_hidden_dim =", self.ed_hidden_dim)
        #     print("[INIT] ed_dim =", self.ed_dim)
        #     print("[INIT] ed_pool =", self.ed_pool)
        #     print("[INIT] ed_k =", self.ed_k)
        #     print("[INIT] ed_encoder =", self.ed_encoder)
        # print("[INIT] protein_atom_emb =", self.protein_atom_emb)
        # print("=" * 80)
    ...
    def interdependency_modeling(
        ...
        """
        基于当前时间步 t 的 belief 表达 θ（θ_h, μ_pos）
        输出目标分布的预测参数 —— 坐标预测值 μ 和原子类别概率 logits
        ...
        Args:
            time: [node_num x batch_size, 1] := [N_ligand, 1]   当前时间步t
            protein_pos: [node_num x batch_size, 3] := [N_protein, 3]   蛋白原子的坐标
            protein_v: [node_num x batch_size, protein_atom_feature_dim] := [N_protein, 27]     蛋白原子的特征
            batch_protein: [node_num x batch_size] := [N_protein]   蛋白原子的批次索引
            theta_h_t: [node_num x batch_size, atom_type] := [N_ligand, 13]     belief 表达的 ligand 原子类型 soft one-hot（discrete θ）
            mu_pos_t: [node_num x batch_size, 3] := [N_ligand, 3]   belief 表达的 ligand 坐标（continuous μ）
            batch_ligand: [node_num x batch_size] := [N_ligand] ligand 原子的 batch id
            gamma_coord: [node_num x batch_size, 1] := [N_ligand, 1]        用于预测真实位置的 reweight 系数
        """
        ...
        # 根据ED修改的h_protein
        # protein_v_aug = torch.cat([protein_v, ed_emb], dim=-1)
        # h_protein = self.protein_atom_emb(protein_v_aug)

        if self.use_ed:
            assert local_ed is not None, "local_ed is None but use_ed=True"

            if local_ed.device != protein_v.device:
                local_ed = local_ed.to(protein_v.device)
            local_ed = local_ed.float()


            # print("=" * 80)
            # print("[DEBUG] protein_v.shape before ED =", tuple(protein_v.shape))
            # print("[DEBUG] local_ed.shape =", tuple(local_ed.shape))
            # print("[DEBUG] local_ed.has_nan =", torch.isnan(local_ed).any().item())
            # print("[DEBUG] local_ed.has_inf =", torch.isinf(local_ed).any().item())

            ed_emb = self.ed_encoder(local_ed)  # [N_protein, ed_dim]

            # print("[DEBUG] ed_emb.shape =", tuple(ed_emb.shape))
            # print("[DEBUG] ed_emb.has_nan =", torch.isnan(ed_emb).any().item())
            # print("[DEBUG] ed_emb.has_inf =", torch.isinf(ed_emb).any().item())

            assert ed_emb.shape[0] == protein_v.shape[0], \
                f"ed_emb and protein_v atom count mismatch: {ed_emb.shape[0]} vs {protein_v.shape[0]}"
            assert ed_emb.shape[1] == self.ed_dim, \
                f"ed_emb dim mismatch: expected {self.ed_dim}, got {ed_emb.shape[1]}"

            protein_v_aug = torch.cat([protein_v, ed_emb], dim=-1)

            # print("[DEBUG] protein_v.shape after ED =", tuple(protein_v_aug.shape))
            # print("=" * 80)
        else:
            protein_v_aug = protein_v
        ...
        final_ligand_v = self.v_inference(final_ligand_h)  # [N_ligand, 13] 将结构特征映射为分布参数的 MLP
        ...
        if not self.destination_prediction:
            ...
            )   # 如果当前时间步还小于最小训练时间步（t_min），就不要 sample，直接返回 0 张量
                # 避免“太早预测”带来的误差累积。这是一个对极端时间步的鲁棒性处理
        else:
    ...
    def reconstruction_loss_one_step(
        ...
        # print("[RECON] local_ed is None =", local_ed is None)
        # if local_ed is not None:
        #     print("[RECON] local_ed.shape =", tuple(local_ed.shape))
    ...
    def loss_one_step(
        ...
        # print("[LOSS_ONE_STEP] local_ed is None =", local_ed is None)
    ...
    def sample(
        ...
        # print("[SAMPLE] local_ed is None =", local_ed is None)
        # if local_ed is not None:
        #     print("[SAMPLE] local_ed.shape =", tuple(local_ed.shape))
