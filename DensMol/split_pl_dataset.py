...
#     parser.add_argument('--path', type=str, default='data/crossdocked_v1.1_rmsd1.0_pocket8')
#     parser.add_argument('--dest', type=str, default='data/crossdocked_pocket8_pose_split.pt')
...
def normalize_protein_filename(fn: str) -> str:
    """
    忽略 pocket 半径后缀，把 _pocket8 / _pocket10 / _pocket12 ... 去掉
    """
    fn = fn.replace("\\", "/")
    fn = re.sub(r"_pocket\d+\.pdb$", ".pdb", fn)
    return fn
...
if __name__ == '__main__':
    ...
    if args.fixed_split:
        ...
        name_id_dict = {}
        for idx, data in enumerate(tqdm(dataset, desc='Indexing')):
            protein_key = normalize_protein_filename(data.protein_filename)
            ligand_key = data.ligand_filename.replace("\\", "/")
            name_id_dict[protein_key + ligand_key] = idx

        selected_ids = {'train': [], 'test': []}
        for split in ['train', 'test']:
            print(f'Selecting {split} split...')
            for fn in fixed_split[split]:
                protein_key = normalize_protein_filename(fn[0])
                ligand_key = fn[1].replace("\\", "/")
                key = protein_key + ligand_key
